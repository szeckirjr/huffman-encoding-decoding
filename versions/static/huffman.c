...
CharCode_T *buildCodeTable(struct MinHuffmanNode *root, int arr[], int binaryCode, int top)
{
    ...
    if (!(root->left) && !(root->right))
    {
        ...
        FILE *fp = fopen("frequency_lut.h", "a");
        fprintf(fp, "#define C%d { .code=0x%x, .len=%d, .ascii=%d, .string=\"%s\" }\n", root->data[0], code, codeLength, root->data[0], root->data);
        fclose(fp);
        currCount++;
    }
...
int main()
{
    char *outputFile = "frequency_lut.h";

    FILE *file = fopen(outputFile, "w");
    fprintf(file, "");

    HuffmanCodes(ENCODE, "decodedText.txt", "output.txt");
